...
def get_menu_data(url, base_url):
    ...
    menu = soup.select(".txt > dt")
    sub = soup.select(".txt > dd")
    menuimage = soup.select("p.img > img")
    price = soup.select(".money")
    chart_data = []
    ...
    return chart_data
tab_urls = [
    "https://www.kyochon.com/menu/chicken.asp",
]
base_url = "https://www.kyochon.com/main/index.asp"
all_tabs_data = []
...
# 데이터를 JSON 파일로 저장
with open("kyochon.json", "w", encoding='utf-8') as json_file:
